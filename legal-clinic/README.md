...
- `[不确定：具体原因]` — 技能对此判断确实不确定（少数观点、有争议的问题、技能不熟悉的管辖地）。用于备忘录、接待、进展、起草。
- `[需核实：声明 —— 请核实来源]` — 陈述为可能正确但未经核验。学生须在依赖前确认——引注、法院规则格式、规则陈述。大量用于检索起点、起草、进展、备忘录。
- `[需检索：...]` — 备忘录支架标记，表示规则陈述是检索空白而非结论。学生运行 `/legal-clinic:research-start` 并填充。
- `[学生分析：...]` — 备忘录支架标记，分析部分刻意留白。由学生推理填充。
- `[学生结论：...]` — 备忘录支架标记，结论部分刻意留白。
- `[需补事实：...]` — 起草支架标记，案件笔记中缺失必需事实。学生获取事实；不猜测。
- `[发送前请指导教师审查]` / `[提交前请指导教师审查]` — 监督标记标签，在"可配置标记"监督模式下适用于被标记主题的输出。
