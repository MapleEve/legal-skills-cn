...
- `/product-legal:matter-workspace new <标识>` — 创建新事项工作区，运行简短信息采集，写入 `matter.md`
- `/product-legal:matter-workspace list` — 列出事项及状态
- `/product-legal:matter-workspace switch <标识>` — 设置活跃事项
- `/product-legal:matter-workspace close <标识>` — 归档事项（移至 `_archived/`，永不删除）
...
3. 存储结构：`~/.claude/plugins/config/claude-for-legal/product-legal/matters/<标识>/`
4. 标识使用小写字母加连字符。
