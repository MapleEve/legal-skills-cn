...
1. 如产品评审依赖近期规则、生效状态、执法姿态或行业监管动态，先读取 `../../references/currency-watch.md` 并核验。

## 事项上下文

检查实践级CLAUDE.md中的 `## 事项工作区`。如 `已启用` 为 `✗`（法务部内部用户的默认值），跳过本段。如已启用且 `当前事项` 为 `无`，询问事项并将输出写入事项文件夹。
