...
**通知方式：** 如果有可用的消息工具集成（飞书/企业微信/内部通知系统），将摘要发送至法务频道。如果不可用，将摘要写入文件（如 `launch-radar-[date].md`）——过滤逻辑与通知渠道无关。

## 功能

1. 读取 `~/.claude/plugins/config/claude-for-legal-cn/product-legal/CLAUDE.md` → 项目管理工具位置、风险校准表、升级渠道。
2. 查询目标日期在30天内的需求单。
...
**可能需要审查：**
* [需求-123] [title] —— 上线日期 [date] —— 匹配 [风险模式]
* [需求-456] [title] —— 上线日期 [date] —— AI组件已识别 —— 需AI治理分诊
* [需求-789] [title] —— 上线日期 [date] —— 提及 [触发关键词] —— 可能需合规审查
