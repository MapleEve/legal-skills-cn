...
- `/ai-governance-legal:matter-workspace switch <slug>` — 设置当前事项
- `/ai-governance-legal:matter-workspace close <slug>` — 归档事项（移至 `_archived/`，永不删除）
- `/ai-governance-legal:matter-workspace none` — 解除任何当前事项的关联，仅在实践级别工作

## 操作说明

1. 读取实践配置 — 确认 `## 事项工作区` 章节已填写。如果 `已启用` 为 `✗`，告知用户事项工作区已关闭——法务部内部用户的默认状态。
2. 使用以下工作流。
3. 根据 `$ARGUMENTS` 的首个词分派：
   - `new` → 运行信息采集访谈，写入 `matters/<slug>/matter.md`，初始化 `history.md` 和 `notes.md`。
   - `list` → 枚举 `matters/*/matter.md`，打印表格，标注当前事项。
   - `switch` → 更新实践级别 CLAUDE.md 中的 `当前事项:` 行。
   - `close` → 移至 `_archived/<slug>/`，在 `history.md` 中记录关闭日期。
   - `none` → 将 `当前事项:` 设为 `无 — 仅实践级别上下文`。
4. 向用户展示变更内容，在写入前确认。
...
## 当前事项在实践级 CLAUDE.md 中

实践级别 CLAUDE.md 中 `## 事项工作区` 下的 `当前事项:` 行是唯一的事实来源。切换事项即编辑该行。
...
### `list`
枚举 `matters/*/matter.md`，打印表格（标识 | 客户 | 事项类型 | 状态 | 开启日期 | 当前），标注当前事项。

### `switch <slug>`
确认存在 → 编辑 `当前事项:` 行 → 展示摘要。

### `close <slug>`
追加关闭记录 → 移至 `_archived/` → 如是当前事项则解除。

### `none`
设置 `当前事项:` 为 `无 — 仅实践级别上下文`。
