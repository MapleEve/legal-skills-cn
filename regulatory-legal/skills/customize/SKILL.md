...
4. **进行更改。** 显示当前值，询问新值，说明下游影响，确认，写入配置。
   ...
   示例：
   - *添加监管机构至关注清单：* "`/regulatory-legal:reg-feed-watcher` 将在下次运行时扫描此监管机构。"
   - *调整重大性标准：* "`/regulatory-legal:reg-feed-watcher` 摘要将更聚焦 — 低于新阈值的项目将移出周报摘要但保持可搜索。"
   - *新增政策加入库：* "`/regulatory-legal:policy-diff` 将纳入此政策。征求意见跟踪器将标记影响此政策的反馈意见。"
