...
- **MCP 连接器**：按各插件内 `.mcp.json` 配置连接器，默认保留已验证的生产力工具和法律检索 MCP；中国法数据库连接器需在真实 MCP 可用后另行配置。
...
| 学习与实训 | [法学生](./law-student) | 问答式案例研习、案例摘要、知识体系、法考训练、IRAC 评分和学习计划；只引导思考，不代写答案。 |
...
> [!IMPORTANT]
> **先确认真实可用的检索连接器。** 各插件按其 `.mcp.json` 配置连接器；默认配置保留已验证的生产力工具和法律检索 MCP。中国法数据库连接器需在真实 MCP 可用后另行配置，不应使用未验证占位项。

| 连接器 | 为 Claude 提供的能力 | 适用插件 | 备注 |
|---|---|---|---|
| **Slack** | 搜索消息、读取频道、查找工作空间讨论 | 多数插件 | 插件内 `.mcp.json` 已配置 |
| **Google Drive** | 搜索、读取和获取云端文档 | 多数插件 | 插件内 `.mcp.json` 已配置 |
| **CourtListener** | 检索美国法院意见、PACER docket、法官资料、口头辩论和引用核验 | 部分检索型插件 | 插件内 `.mcp.json` 已配置 |
| **Descrybe** | 按概念或措辞检索案例、从 citation 找案例、提取 authority、检查引用处理 | 部分检索型插件 | 插件内 `.mcp.json` 已配置 |
| **Solve Intelligence** | 专利、非专利文献、法律文本、SEP 标准、现有技术和权利要求分析 | `知识产权` | 插件内 `.mcp.json` 已配置 |
| **Everlaw** | 搜索和读取电子证据项目文档、元数据和 review 链接 | `争议解决` | 插件内 `.mcp.json` 已配置 |
| **Aurora** | 只读电子证据检索、事项和 workspace 查询、跨事项调查 | `争议解决` | 插件内 `.mcp.json` 已配置 |
| **Trellis** | 州审判法院 docket、裁判、判决、filing 和法官 / 对方律师分析 | `争议解决` | 插件内 `.mcp.json` 已配置 |
| **TopCounsel** | 外部律师推荐和专业能力证据 | `争议解决` | 插件内 `.mcp.json` 已配置 |
| **Courtroom5** | 面向自助诉讼人的案件接案、期限计算和程序下一步指引 | `法律援助工作站` | 插件内 `.mcp.json` 已配置 |
| **中国法数据库连接器** | 法规、案例、实务指南和来源核验 | 按真实 MCP 能力另行配置 | 真实 MCP 可用后再配置；不要使用未验证占位项 |
