...
- 准备把 Claude 接进内部系统的人：需要 MCP 连接器、托管智能体手册、办公文档侧边栏和后台编排参考。
...
> [!IMPORTANT]
> **首先接入法律检索工具。** 各插件按其 `.mcp.json` 配置连接器；默认配置面向中国法律实务，覆盖通用办公系统、法律检索、合同管理、电子签约和案件材料管理等能力。连接器使引用可信，在任何其他配置之前先配好它们。
