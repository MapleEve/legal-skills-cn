...
1. 读取`~/.claude/plugins/config/claude-for-legal-cn/employment-legal/CLAUDE.md`→管辖表和系统部分。
...
5. 使用假期登记册格式将新条目写入`~/.claude/plugins/config/claude-for-legal-cn/employment-legal/leave-register.yaml`。如果文件不存在，创建它。YAML格式：
...
6. 用以下确认信息回复：
   ...
   > **工资待遇：** [适用标准]
   >
   > 假期追踪器将自动在截止日期临近时发出提醒。运行 `/employment-legal:leave-tracker` 随时查看所有活跃假期。
