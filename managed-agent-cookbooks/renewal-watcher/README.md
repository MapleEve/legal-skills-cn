# 续约监控器（Renewal Watcher）—— 自动化工作流模板

## 概述

扫描合同库中即将到来的续约和解约截止日，与团队审查手册交叉比对，标记有即将到期、审查手册偏离和逐级上报触发条件的合同，并撰写预警报告。本目录为 `POST /v1/agents` 的自动化工作流模板，技能仍来自 commercial-legal 插件；本模板的 system text 在本目录内维护，以保持中文合同管理和本地投递默认项。
