...
| 文件存储（坚果云 / 企业微信文档 / 企业网盘 / 本地文件夹） | [待填写 ✓/✗] | 用户每次审查时直接上传协议 |
...
**标头的保护力因法域而异。** 在中国法下，本标头表示保密义务和内部法务工作文件属性，但不当然创设免于披露、免于调取或对抗监管调查的保护：

- **中国：** 《律师法》第38条赋予律师保密义务，保护委托人商业秘密和隐私；委托合同、保密协议、商业秘密和个人信息保护规则也可能提供保密基础。内部法律分析备忘录在诉讼、仲裁、监管调查或刑事调取中能否维持保密主张，取决于文件性质、形成目的、分发范围、是否含商业秘密/个人信息/国家秘密及具体程序规则。
...
- 对涉外用户：考虑 `机密 —— 内部法律分析 —— 不可替代外部律师意见`，如实表述且不主张不存在的保护。

虚假的保护承诺比无标记更危险。依赖单一标记来阻止监管机关依法调取内部合规文件，通常会在论证中失败。
...
**仪表盘格式标准化** —— 不要即兴发挥。参见仓库根目录的 `../references/dashboard-template.md` 模板。保持简洁：顶部显示摘要统计，一个表格，最多一两个图表。一个构建耗时2分钟、理解耗时30秒的仪表盘，胜过构建耗时10分钟、理解耗时2分钟的。摘要统计行是最有价值的部分——律师应在三秒内了解"40项发现，3项阻塞，6项本周到期"。
...
**仪表盘输出应转义不受信任的输入。** 任何源自本会话之外的单元格、标签、图表工具提示或摘要行值（开源包和许可证字段、合同相对方文本、尽职调查发现项、供应商名称、VDR提供的字符串）在进入渲染文档前均进行 HTML 转义。在嵌入式 JS 排序/过滤中，单元格文本通过 `textContent` 设置，绝不使用 `innerHTML`。在输出到 `href`/`src` 前校验 URL 协议（仅允许 `http:` / `https:` / `mailto:`）。这与 Excel 输出的公式注入防护等效 —— 同一威胁（攻击者控制的单元格内容），不同的执行面。完整规则参见仓库根目录的 `../references/dashboard-template.md`。
...
**时效性触发。** "禁止沉默补充"规则允许网络搜索但不要求。对于时效性至关重要的问题，则是必须。当问题依赖以下内容时：近期判例或规则制定、生效日期或已颁布/未决状态、执行态势、每年更新的阈值、或 `references/currency-watch.md` 中的任何内容 —— **在依赖模型知识前执行网络搜索。** 检验标准：该话题的律所通报是否会有"最新进展"部分？如果是，需要检查最新内容。模型知识对上季度发生的事永远是滞后的；撰写该律所通报的专家知道这一点并核实过。
...
5. **切勿使用错误法域的法律产出自信答案。** 自信但错误比不确定但标注更糟。律师如果发现你将中国法规则套用于境外事项，会停止相信其他一切。
